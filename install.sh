...
print_final_instructions() {
    ...
    echo ""
    echo -e "${NVIDIA_GREEN}2.${RESET} ${BOLD}After reboot, verify virtual display${RESET}"
    echo -e "${GRAY}   └─${RESET} Run: ${DIM}xrandr${RESET}"
    echo -e "${GRAY}   └─${RESET} Look for: ${RESOLUTION} @ ${REFRESH_RATE}Hz"
    echo ""
    echo -e "${NVIDIA_GREEN}3.${RESET} ${BOLD}Start Sunshine service${RESET}"
    echo -e "${GRAY}   └─${RESET} Run: ${DIM}systemctl --user start sunshine${RESET}"
    echo ""
    echo -e "${NVIDIA_GREEN}4.${RESET} ${BOLD}Configure Sunshine credentials${RESET}"
    echo -e "${GRAY}   └─${RESET} Open: ${DIM}https://localhost:47990${RESET}"
    echo -e "${GRAY}   └─${RESET} Set username and password"
    echo ""
    echo -e "${NVIDIA_GREEN}5.${RESET} ${BOLD}Connect with Moonlight client${RESET}"
    echo -e "${GRAY}   └─${RESET} Download: ${DIM}https://moonlight-stream.org${RESET}"
    echo -e "${GRAY}   └─${RESET} Scan for ${HOSTNAME} on your network"
    echo ""
    echo -e "${WHITE}${BOLD}Troubleshooting:${RESET}"
    echo -e "${GRAY}  •${RESET} Check Sunshine logs: ${DIM}journalctl --user -u sunshine -f${RESET}"
    echo -e "${GRAY}  •${RESET} Verify GPU: ${DIM}nvidia-smi${RESET}"
    echo -e "${GRAY}  •${RESET} Check display: ${DIM}xrandr${RESET}"
    echo ""
    echo -e "${WHITE}${BOLD}Backups Location:${RESET}"
